...
class SmsReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        try {
            if (bundle != null){
//               Bundle dengan key pdus adalah standar dari system
                val pdusObj = bundle.get("pdus") as Array<*>
                for (aPdusObj in pdusObj){
                    val currennMessage = getIncomeMessage(aPdusObj as Any, bundle)
                    val senderNum = currennMessage.displayOriginatingAddress
                    val message = currennMessage.displayMessageBody
